...
int repeat_main(int argc, char** argv)
{
	...
	//OLD multInf.removeUnsupportedEdges();
	//OLD multInf.maskUnsupportedEdges();
	//OLD multInf.removeUnsupportedConnections();
	//OLD ADDED outGen.outputDot(proc.getEdgesPaths(), outFolder + "/graph_before_coll.gv");
	//OLD ADDED rg.storeGraph(outFolder + "/before_coll_repeat_graph_dump");
	//OLD ADDED aligner.storeAlignments(outFolder + "/before_coll_read_alignment_dump");
	//OLD ADDED SequenceContainer::writeFasta(edgeSequences.iterSeqs(), 
	//OLD ADDED							  outFolder + "/before_coll_repeat_graph_edges.fasta",
	//OLD ADDED							  /*only pos strand*/ true);


	//OLD multInf.collapseHeterozygousLoops(/*remove alternatives*/ false);
	//OLD multInf.collapseHeterozygousBulges(/*remove alternatives*/ false);
